...
To correct for the distortion we made use of OpenCVs (\cite{noauthor_opencv/opencv_2021}) camera calibration toolbox.

In order to correct for distortion, the camera matrix and distortion coefficients are needed. To do this, we used the calibrate script provided with OpenCV,
...
We found that our offline OpenDataCam setup using an Nvidia Jetson NX device worked well for detecting cars, but did not perform as ell on cyclists. 
...
YOLO does not require such powerful hardware, less powerful hardware would also work.
\ \\

With this setup, videos were processed at ~10 FPS. This results in a total processing time of ~4.5 hours for a 1.5-hour video.
...
We made a function that allows for the selection of a polygon on the source and destination image, the points
of the polygon
